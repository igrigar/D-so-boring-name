...
int createFile(char *fileName)
{
	...
	inodes[i].size = 1; // Al menos tiene el puntero al siguiente bloque (-1).
...
int readFile(int fileDescriptor, void *buffer, int numBytes)
{
	...
	if ((SUB_BLOCK_SIZE - pointer) < numBytes) { // Caso mitad dos bloques.
		int read = SUB_BLOCK_SIZE - pointer; // Cuanto se va a leer 1er bloque.
...
int writeFile(int fileDescriptor, void *buffer, int numBytes)
{
	...
	if (pointer + numBytes >= SUB_BLOCK_SIZE &&// Escribimos al menos un sub-bloque entero.
			(inodes[openFiles[fileDescriptor][0]].size + numBytes) < MAX_FILE_SIZE
			&& sb->next < 0 ) {
...
int  bmap(int i, int offset) {
	...
	if (offset < 0) return -1; // Mal despazamiento.

	if (offset < ((BLOCK_SIZE/2) - 1))
		return inodes[i].firstBlock; // Puntero en primer bloque.
	...
	/*
	// Forma 'genérica' de recorrer lista de bloques enlazados.
	// Calculando el sub-bloque en el que esta.
	int blockOffset = offset/SUB_BLOCK_SIZE;
	int blockNum = inodes[i].firstBlock;
	...
	for (int i = 0; i <= blockOffset; i++) {
		if (bread(DEVICE_IMAGE, blockNum/2 + superBlock[0].firstData, block) == -1)
			return -1;

		offset = (blockNum%2) * sizeof(block_t);
		strncpy(subBlock, (char *) block + offset, sizeof(block_t));
		sb = (block_t *) subBlock;
		if (sb->next == -1) return -1; // El offset es demasiado grande?
	}
	*/
...
uint32_t computeCRC(int i, int type) {
	...
	switch (type) {
		...
		case CRC_FILE:
			...
			// Calculamos cuantos bloques debería tener el archivo.
			int numBlock = (inodes[i].size/SUB_BLOCK_SIZE) + 1;

			// Reservamos la memoria en bytes. Le sumamos el numBlock porque
			// tiene tantos bytes de metadatos (puntero a siguiente bloque) como
			// bloques.
			size = numBlock * SUB_BLOCK_SIZE + numBlock;
			buffer = malloc(size);

			// Guardamos la referencia al bloque directo.
			int blockNum = inodes[i].firstBlock;

			// Cogemos los datos de todos los sub-bloques que haya.
			for (int i = 0; i < numBlock; i++) {
				if (bread(DEVICE_IMAGE, blockNum/2 + superBlock[0].firstData,
					block) == -1) return -1;
				...
				strncpy((char *) buffer + (i * SUB_BLOCK_SIZE + 1), (char *) sb,
					SUB_BLOCK_SIZE + 1);

				// Obtenemos el siguiente sub-bloque.
				blockNum = sb->next;
			}
